...
function NavItem({ item, active }: { item: Item; active: boolean }) {
  ...
  const inactive = "text-bb-muted hover:text-bb-text hover:bg-bb-surface/40";
  ...
  return (
    <div className="p-4 flex h-full flex-col gap-4">
      <div className="rounded-2xl bg-bb-surface/50 border border-bb-border/5 p-4 shadow-soft">
        <div className="text-xs text-bb-muted tracking-widest">BAZAARBAH</div>
      ...
      <div className="mt-auto rounded-2xl bg-bb-surface/40 border border-bb-border/5 p-3">
        <div className="text-xs text-bb-muted">Plan</div>
        <div className="text-sm">Basic • RM88</div>
      </div>
    </div>
  );
}
